...
from backend.core.geo import EARTH_RADIUS_M, haversine_dist


GEOMETRY_PATH = Path(__file__).with_name("resources") / "lemd_runways_2026-07-09.json"
...
@lru_cache(maxsize=1)
def load_lemd_geometry(path: str | Path = GEOMETRY_PATH) -> GeometryCatalog:
    artifact = Path(path)
