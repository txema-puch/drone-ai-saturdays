...
Phase 4 covers the structural EDA needed to inform Phase 3's preprocessing pipeline definition. The work spans two rounds of iteration documented in `notebooks/06_phase4_eda.ipynb` (single notebook, append-only with retrospective markers):
...
The full methodology trail (questions asked, hypotheses tested, decisions made, retrospective on C2 retirement, multi-detector architecture derivation, sanity-cohort design) lives in `notebooks/06_phase4_eda.ipynb`'s concluding narrative section (cell 40).
...
- Notebook: `notebooks/06_phase4_eda.ipynb`. "Run All" in Jupyter or VSCode to regenerate all figures.
...
- Figures persisted to: `backend/docs/ml/figures/04-eda/` (7 figures for Insights 1-7 plus Phase 0 validation figures).
