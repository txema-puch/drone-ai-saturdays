...
OpenSky Dataset #6 — 832 flights that squawked **7700** (Olive et al., 2020) — is our Layer-4 external-validation set (D-008). The Phase-4 inspection (`notebooks/08_phase4_dataset6_emergency_eda.ipynb`) found only **N=6** LEMD-associated flights by airport code. That leaves **~826 non-LEMD emergency flights** otherwise unused.
...
Graft the relative delta sequence onto the matching phase of a LEMD-normal trajectory, **ramping in at the splice** (the `_ramp` discipline in `backend/docs/ml/references/sadar_synthetic_bench.py`) to avoid discontinuities. Resample source 1 s → 10 s before extraction. Produce **ground-truth onset masks** (→ detection-latency metric). Re-apply the **train-fit scaler** (never re-fit).
...
- `backend/docs/ml/dataset6-emergency-external-validation.md` — Dataset #6 acquisition/firewall spec (§8 open question resolved here).
- `notebooks/08_phase4_dataset6_emergency_eda.ipynb` — the N=6 LEMD finding.
- `backend/docs/ml/references/sadar_synthetic_bench.py` — the injection scaffold (ramp, onset masks).
- `backend/docs/writeup/09-the-architectural-critique.md` — the framing + per-type comparison this feeds.
