...
**Code:** `backend/core/features.py` (+ contract promotion in `backend/core/preprocessing.py`) · **Tests:** `backend/tests/test_features.py`
...
The single source of truth is `backend/core/preprocessing.py` — `AE_FEATURES` /
...
- Code: `backend/core/features.py`; contract in `backend/core/preprocessing.py`.
