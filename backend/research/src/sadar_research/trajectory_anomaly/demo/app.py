...
BUNDLE = Path(os.environ["SADAR_RESEARCH_BUNDLE_DIR"])
MODELS = Path(os.environ["SADAR_RESEARCH_MODELS_DIR"])
FRONTEND_DIST = Path(os.environ["SADAR_FRONTEND_DIR"])
...
def _load_bundle():
    if CONFIGURED_RELEASE_DIR:
        from sadar_research.trajectory_anomaly.releases.schema import SERVING_FILE_BYTE_LIMITS, read_json_file, validate_release_directory
...
def _load_runtime_artifacts():
    if CONFIGURED_RELEASE_DIR:
        from sadar_research.trajectory_anomaly.releases.artifacts import load_model_artifacts
        from sadar_research.trajectory_anomaly.evaluation.upload import (
...
from sadar_research.trajectory_anomaly.demo.runtime import AnalysisBusy, ModelNotReady, ModelRuntime  # noqa: E402
...
from sadar_research.trajectory_anomaly.evaluation.upload import MAX_MULTIPART_BYTES  # noqa: E402
app.add_middleware(
...
def simulate(request: SimulationRequest) -> dict:
    ...
    from sadar_research.trajectory_anomaly.evaluation.scoring import simulate_segment
