...
function CohortTables({ cohort }: { cohort: ResearchCohort }) {
  return (
    <article className="evidence-cohort">
      ...
      <div className="evidence-table-scroll" tabIndex={0} aria-label={`${cohort.period} criterion counts table`}>
        <table>
          <caption>Criterion status counts</caption>
          <thead><tr><th scope="col">Criterion</th><th scope="col">Status</th><th scope="col">Attempts</th></tr></thead>
          <tbody>{Object.entries(cohort.criterion_status_counts).flatMap(([criterion, values]) => (
