...
- `docs/research/trajectory-anomaly/scoping/README.md` — D-001 to D-004 (use case, modality, dataset, scope)
- `/ml-lifecycle` Phase 1 reference + guardrails (testing-firewall, baseline-required, etc.)

## Outputs

- `docs/research/trajectory-anomaly/lifecycle/manifest.yml` — machine-readable state
- This document — narrative rationale
- `docs/research/trajectory-anomaly/lifecycle/decisions/D-001-anomaly-vs-classification.md` — framing ADR
- `docs/research/trajectory-anomaly/lifecycle/decisions/D-005-metric-stack.md` — metric stack ADR
- `docs/research/trajectory-anomaly/lifecycle/decisions/D-006-architecture-and-baseline.md` — architecture ADR
...
The operational restatement is: **a behavioral anomaly detector for cooperating aircraft at LEMD, designed to catch sequence-shaped behavioral deviations (hovering, speed spikes, late-trajectory deviation) that the deployed rule-based safety nets do not.** The model scores **complete trajectories in batch**, so its realistic user is *retrospective* — surveillance-side flight-data monitoring / post-operations safety review — not the real-time controller, whom the deployed nets already serve; complementarity is by **time horizon**, not by competing for the same instant. Per-anomaly-type expectation: AE loses to safety-net analogs on zone violation (APW) and altitude violation (MSAW); AE wins on hovering, speed spike, and late-trajectory deviation.

This reframe is the thesis of the Medium piece — see the archived architectural-critique draft listed in `docs/archive-manifest.yml`. It does not invalidate the metric stack, baseline, or architecture choices below; those remain operational. It does narrow what the model can honestly claim to detect, which the writeup carries explicitly. D-006 carries a parallel interpretation note.
...
| D-002 | 2026-04-11 | ADS-B as training modality; source-agnostic model at inference | [project decisions log](../scoping/README.md) |
| D-003 | 2026-04-11 | OpenSky Network as primary dataset | [project decisions log](../scoping/README.md) |
| D-004 | 2026-04-11 | Anomaly scoring only; no trajectory prediction | [project decisions log](../scoping/README.md) |
...
- [x] `docs/research/trajectory-anomaly/lifecycle/01-problem.md` exists, all sections filled in
