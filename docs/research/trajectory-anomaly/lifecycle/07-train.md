...
**Issue:** #27 · **Design:** `docs/research/trajectory-anomaly/lifecycle/07-train.md`
...
`research/trajectory-anomaly/notebooks/lifecycle/11_phase6_loopback.ipynb` on the fixed modules to refresh. The conclusion is unchanged
...
val firewall** (fit on TRAIN-normal, score val, TEST sealed; `research/trajectory-anomaly/notebooks/lifecycle/12_density_baselines.ipynb`):
