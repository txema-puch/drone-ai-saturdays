...
Phase 4 EDA (`docs/research/trajectory-anomaly/lifecycle/04-eda.md`, `research/trajectory-anomaly/notebooks/lifecycle/06_phase4_eda.ipynb`) produced two intertwined Phase 3 decisions that together define the preprocessing pipeline: **which trajectories enter training** (the LEMD-operation gate) and **how to handle gross sensor outliers within those trajectories** (the kinematic-impossibility handling rule).
...
- The project's post-Phase-1 architectural critique (the archived architectural-critique draft listed in `docs/archive-manifest.yml`, formal references in `01-problem.md > Scope evolution` and `D-006 > Scope reframe (post-Phase 1)`) reframed the model as **behavioral anomaly detection on cooperating aircraft**. Under that framing, C2 was wrong: it excluded legitimate operational variety (holds, missed approaches, go-arounds) that the model will see at inference.
...
- **Phase 3 entry artifact** (to be written): `docs/research/trajectory-anomaly/lifecycle/03-preprocess.md` will codify the three-stage pipeline as `pipeline_definition`.
- **Code-level implementation** (Phase 3 deliverable): `backend/research/src/sadar_research/trajectory_anomaly/pipeline/preprocessing.py` (or similar) implements:
...
- **Phase 4 evidence base:** `docs/research/trajectory-anomaly/lifecycle/04-eda.md` (current state) and `research/trajectory-anomaly/notebooks/lifecycle/06_phase4_eda.ipynb` (full methodology trail including the C2-to-D evolution and the multi-detector architecture derivation).
- **Phase 7 sanity cohort consumer:** D-008 Layer 1 (`docs/research/trajectory-anomaly/lifecycle/decisions/D-008-output-validation-layers.md > Layer 1 — sanity cohort, in detail`).
- **Manifest cross-references** (to be updated when Phase 3 closes):
  - `manifest.yml > gates.preprocess.artifact = docs/research/trajectory-anomaly/lifecycle/03-preprocess.md`
...
Implementation (`backend/research/src/sadar_research/trajectory_anomaly/pipeline/preprocessing.py`, notebook `07` Findings A–E, codex
...
the full ordered spec lives in `docs/research/trajectory-anomaly/lifecycle/03-preprocess.md`.
