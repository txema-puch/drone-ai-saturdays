...
primary; the existing `OpenSkyService` in `backend/research/src/sadar_research/trajectory_anomaly/data/opensky.py` stays in
...
- **Filter B** applied per trajectory: keep iff `min(dist_to_runway_m) < 10_000m`
  ...
  See `backend/research/src/sadar_research/trajectory_anomaly/data/download.py` docstring.
...
- New script: `backend/research/src/sadar_research/trajectory_anomaly/data/download.py` (~370 lines).
  Documented inline; reuses `distance_to_closest_runway` and
  `calculate_flight_phase` from `backend/research/src/sadar_research/trajectory_anomaly/data/opensky.py` to keep derivation
...
- Pre-existing settings-loader bug fixed: `Settings.Config.extra = "ignore"`
  in `backend/research/src/sadar_research/trajectory_anomaly/data/config.py` so the multi-account cycle-N env vars from
...
- [D-002 — ADS-B as training modality; source-agnostic model at inference](../../scoping/README.md)
- [D-003 — OpenSky Network as primary dataset](../../scoping/README.md)
- [D-209 — Parquet naming derives from data's `time_utc` range](../02-data.md)
- [D-210 — Six-category response playbook for audit findings](../../data-workflow.md)
