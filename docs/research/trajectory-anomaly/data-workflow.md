...
>   `backend/research/src/sadar_research/trajectory_anomaly/data/download.py` pulls public S3 data
...
```
   OpenSky Network (historical archive, Trino)
                ...
                ▼
   backend/research/src/sadar_research/trajectory_anomaly/data/export_sample.py    ← Monica
   ...
   Supabase (a single working table — accumulates rows
             ...
             constraint prevents within-cycle duplicates)
                ...
                ▼
   research/trajectory-anomaly/notebooks/lifecycle/05_phase2_data_validation.ipynb     ← Txema
...
- **Outputs not shown in the flow:**
  - Per-feature histograms to `docs/research/trajectory-anomaly/figures/02-data/`.
  - Printed verdict and counts that **a human transcribes** into
    `docs/research/trajectory-anomaly/lifecycle/02-data.md`'s snapshot log and the
...
- Maintains `backend/research/src/sadar_research/trajectory_anomaly/data/export_sample.py` and the
  `OpenSkyService` class in `backend/research/src/sadar_research/trajectory_anomaly/data/opensky.py`
...
- Runs `research/trajectory-anomaly/notebooks/lifecycle/05_phase2_data_validation.ipynb` against each new
  Supabase batch
...
- Updates `docs/research/trajectory-anomaly/lifecycle/manifest.yml > gates.data.dataset_hash`
  with the new sha256 entry
- Documents any anomalies in `docs/research/trajectory-anomaly/lifecycle/02-data.md` and opens
...
- [ ] Run `research/trajectory-anomaly/notebooks/lifecycle/05_phase2_data_validation.ipynb` against the
...
### Script
`backend/research/src/sadar_research/trajectory_anomaly/data/download.py` — ~370 lines, runs
...
`calculate_flight_phase` from `backend/research/src/sadar_research/trajectory_anomaly/data/opensky.py` — same
...
`research/trajectory-anomaly/notebooks/lifecycle/05_phase2_data_validation.ipynb` for cycle 3, or treat the
...
in `docs/research/trajectory-anomaly/scoping/README.md`.
